      ...
      <div className="admin-split">
        ...
        <section className="admin-panel">
          ...
          {metrics.lowStock.length ? <div style={{padding:12}}>{metrics.lowStock.slice(0, 7).map((product) => {
            const qty = (product.sizes || []).reduce((sum, size) => sum + Number(size.stock || 0), 0);
            return <SiteLink href={`/admin/products?search=${encodeURIComponent(product.sku || product.name)}`} key={product.id} style={{display:'flex',alignItems:'center',gap:12,padding:10,textDecoration:'none',color:'#171717',borderBottom:'1px solid #eee'}}><img src={product.images?.[0]} alt="" style={{width:44,height:44,objectFit:'contain',background:'#f4f4f4'}}/><span style={{flex:1,fontSize:11}}><b style={{display:'block',textTransform:'uppercase'}}>{product.name}</b><small style={{color:'#888'}}>{product.brand} · {product.sku}</small></span><strong style={{color:qty === 0 ? '#c82027' : '#e47a19'}}>{qty}</strong></SiteLink>;
          })}</div> : <EmptyState title="Stock looks healthy" text="No products are currently below the low-stock threshold." />}
        </section>
      </div>

      <section className="admin-card-grid">
        <SiteLink className="admin-card" href="/admin/orders" style={{textDecoration:'none',color:'#171717'}}><b>Process orders</b><p style={{color:'#777',fontSize:12,lineHeight:1.6}}>Confirm, ship, and complete customer orders.</p><span style={{color:'#e31b23',fontSize:11,fontWeight:800}}>Open orders →</span></SiteLink>
        <SiteLink className="admin-card" href="/admin/products" style={{textDecoration:'none',color:'#171717'}}><b>Manage inventory</b><p style={{color:'#777',fontSize:12,lineHeight:1.6}}>Add products, images, sizes, prices, and stock.</p><span style={{color:'#e31b23',fontSize:11,fontWeight:800}}>Open products →</span></SiteLink>
        <SiteLink className="admin-card" href="/admin/slider" style={{textDecoration:'none',color:'#171717'}}><b>Update homepage</b><p style={{color:'#777',fontSize:12,lineHeight:1.6}}>Control the promotional slides customers see first.</p><span style={{color:'#e31b23',fontSize:11,fontWeight:800}}>Edit homepage →</span></SiteLink>
      </section>
    </AdminLayout>
  );
}
