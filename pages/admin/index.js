      ...
      <div style={{minHeight:'100vh',background:'#F8F8F8',fontFamily:'Inter, sans-serif'}}>
        ...
        <div style={{maxWidth:1400,margin:'0 auto',padding:'28px 24px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:12,marginBottom:28}}>
            {[
              {l:'Products',v:totalProducts,c:'#000'},{l:'In Stock',v:inStock,c:'#16A34A'},{l:'Low Stock',v:lowStock.length,c:'#EA580C',a:lowStock.length>0},
              {l:'Out of Stock',v:outOfStock.length,c:'#DC2626',a:outOfStock.length>0},{l:'Total Orders',v:orders.length,c:'#2563EB'},
              {l:'Pending',v:pendingOrders.length,c:'#EA580C',a:pendingOrders.length>0},{l:'Today Revenue',v:todayRevenue.toLocaleString()+' MKD',c:'#16A34A'},
              {l:'Total Revenue',v:totalRevenue.toLocaleString()+' MKD',c:'#000'},{l:'Customers',v:totalCustomers,c:'#000'},
            ].map(s=>(
              <div key={s.l} style={{background:s.a?'#FFF7ED':'#FFF',padding:'16px 18px',borderRadius:2,border:'1px solid #F0F0F0',borderLeft:`3px solid ${s.c}`}}>
                <p style={{fontFamily:'Montserrat, sans-serif',fontSize:26,fontWeight:900,margin:'0 0 4px',color:s.c}}>{s.v}</p>
                <p style={{color:'#999',fontSize:10,fontWeight:700,letterSpacing:1,textTransform:'uppercase',margin:0}}>{s.l}</p>
              </div>
            ))}
          </div>
          ...
          {/* Low Stock Alert */}
          {lowStock.length>0&&(
            <div style={{background:'#FFF7ED',border:'1px solid #FED7AA',padding:18,borderRadius:2,marginBottom:24}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                <div><h2 style={{fontFamily:'Montserrat, sans-serif',fontSize:13,fontWeight:900,color:'#EA580C',margin:0}}>Low Stock Alert ({lowStock.length})</h2><p style={{color:'#9A3412',fontSize:11,margin:'2px 0 0'}}>Restock needed</p></div>
                <a href="/admin/products" style={{color:'#EA580C',textDecoration:'none',fontSize:11,fontWeight:700}}>Manage →</a>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:8}}>
                {lowStock.map(p=> <div key={p.id} style={{background:'#FFF',padding:12,borderRadius:2,display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><p style={{fontWeight:700,fontSize:11,textTransform:'uppercase',margin:'0 0 2px'}}>{p.name}</p><p style={{color:'#DC2626',fontSize:10,fontWeight:700,textTransform:'uppercase',margin:0}}>{p.brand}</p></div><span style={{background:'#FEF2F2',color:'#DC2626',fontWeight:900,fontSize:20,padding:'6px 14px',borderRadius:2}}>{p.sizes.reduce((s,x)=>s+(x.stock||0),0)}</span></div>)}
              </div>
            </div>
          )}
          ...
          <div style={{background:'#FFF',border:'1px solid #F0F0F0',borderRadius:2,overflow:'hidden'}}>
            <div style={{padding:'16px 20px',borderBottom:'1px solid #F0F0F0',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8}}>
              <div style={{display:'flex',gap:0}}>
                {['overview','pending','shipped','delivered'].map(tab=> <button key={tab} onClick={()=>setActiveTab(tab)} style={{padding:'7px 16px',cursor:'pointer',fontWeight:700,fontSize:10,letterSpacing:1,textTransform:'uppercase',background:'none',border:'none',color:activeTab===tab?'#DC2626':'#999',borderBottom:activeTab===tab?'2px solid #DC2626':'2px solid transparent',fontFamily:'Inter, sans-serif'}}>{tab}</button>)}
              </div>
              <a href="/admin/orders" style={{color:'#000',textDecoration:'none',fontSize:11,fontWeight:700}}>View All →</a>
            </div>
            {displayOrders.length===0?<p style={{color:'#999',fontSize:12,textAlign:'center',padding:40,margin:0}}>No orders</p>:
              displayOrders.map(o=>(
                <div key={o.id} style={{padding:'12px 20px',borderBottom:'1px solid #F5F5F5',display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
                  <span style={{fontWeight:700,fontSize:12,minWidth:70}}>#{String(o.id).slice(-6)}</span>
                  <span style={{color:'#999',fontSize:10,minWidth:80}}>{new Date(o.createdAt||o.created_at).toLocaleDateString()}</span>
                  <span style={{fontWeight:600,fontSize:12,flex:1,minWidth:100}}>{o.customer_name||o.customerInfo?.fullName||'Unknown'}</span>
                  <span style={{fontSize:11,minWidth:120}}>{o.product_name||o.product?.name||'-'}</span>
                  <span style={{fontWeight:700,fontSize:12,minWidth:70}}>{o.total||0} MKD</span>
                  <span style={{background:statusBg(o.status),color:statusColor(o.status),padding:'2px 8px',fontSize:9,fontWeight:700,textTransform:'uppercase',borderRadius:2}}>{o.status||'Pending'}</span>
                  {o.status==='pending'&&<button onClick={()=>updateOrderStatus(o.id,'shipped')} style={{background:'#000',color:'#FFF',border:'none',padding:'4px 12px',fontSize:9,fontWeight:700,cursor:'pointer',borderRadius:2}}>Ship</button>}
                  {o.status==='shipped'&&<button onClick={()=>updateOrderStatus(o.id,'delivered')} style={{background:'#2563EB',color:'#FFF',border:'none',padding:'4px 12px',fontSize:9,fontWeight:700,cursor:'pointer',borderRadius:2}}>Deliver</button>}
                </div>
              ))}
          </div>
        </div>
